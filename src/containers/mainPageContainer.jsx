        ...
        <div className='container'>
          <div className="center-block">
            ...
            <div className='btn-group marginBotStandart col-md-12 nonePadding'>
              <button onClick={() => this.goToState('/create-competition')} className='btn btn-info heightButton col-md-2'>
              ...
              <table className="table table-condensed table-striped table-hover ">
                <thead>
                  <tr className="info active">
                    <th className="text-center col-md-2" onClick={() => this.sortNameCompetitions()}>NAME COMPETITION
                      <span class="fa fa-sort float-right"></span></th>
                    <th className="text-center col-md-2" onClick={() => this.sortDateCompetitions()}>DATE START COMPETITION
                      <span class="fa fa-sort float-right"></span></th>
              ...
              <div className='row-md-2 heightButton col-md-6 col-md-offset-4'>
                ...
                <div className="col-md-2">
                  <select className="btn btn-default heightButton"
                          onChange={event => this.setState({pageNo: event && event.target && event.target.value ? event.target.value : null})}
                          onClick={() => this.getCompetitionInfo()} value={this.state.pageNo ? this.state.pageNo : ''}>
                    {
                ...
                <div className='btn-group'>
                  {
                    !!this.state.pageNo &&
                    <button className='btn heightButton col-md-1 colMargin' onClick={() => this.goToPrev()}>
                      <span class="fas fa-angle-left"></span></button>
                  }

                  {
                    !this.state.pageNo &&
                    <button disabled className='btn heightButton col-md-1 colMargin' onClick={() => this.goToPrev()}>
                      <span class="fas fa-angle-left"></span></button>
                  }
                  {
                    this.state.pageNo < this.state.totalPages - 1 &&
                    <button className='noneFloat btn heightButton col-md-1 colMargin' onClick={() => this.goToNext()}>
                      <span class="fas fa-angle-right"></span></button>
                  }

                  {
                    this.state.pageNo >= this.state.totalPages - 1 &&
                    <button disabled className='noneFloat btn heightButton col-md-1 colMargin' onClick={() => this.goToNext()}>
                      <span class="fas fa-angle-right"></span></button>
                  }
              </div>
            </div>
